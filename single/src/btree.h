...
struct timespec clwb_start, clwb_end;
...
int clflush_cnt = 0;
int node_cnt=0;
long long clflushtime;
long long insert_time;
long long keyinsert_time;
...
*/
/*
inline void pmemobj_persist(PMEMobjpool* pop, void *data, int len) {
        ...
        struct timespec start, end;
          ...
          keyinsert_time += (end.tv_sec - start.tv_sec) * 1000000000 + (end.tv_nsec - start.tv_nsec);
}
*/
class btree{
  private:
...
//            if(num_entries_before == 1 && (hdr.sibling_ptr.oid.off == 0)) {
            if(num_entries_before == 1 && (hdr.sibling_ptr.oid.off != 0)) {
              ...
              pmemobj_persist(bt->pop, (char*)&(bt->root), sizeof(TOID(page)));
...
//      TOID(page) left_sibling = bt->root;
      ...
      if(is_leftmost_node) {
        D_RW(hdr.sibling_ptr)->remove(bt, D_RW(hdr.sibling_ptr)->records[0].key, true,
            with_lock);
        return true;
      }
      ...
      if(total_num_entries > cardinality - 1) { // Redistribution
        ...
        if(num_entries < left_num_entries) { // left -> right
          if(hdr.leftmost_ptr == nullptr){
            ...
            pmemobj_persist(bt->pop, (char*)&(D_RW(left_sibling)->records[m].ptr), sizeof(char *));

            D_RW(left_sibling)->hdr.last_index = m - 1;
            pmemobj_persist(bt->pop, (char*)&(D_RW(left_sibling)->hdr.last_index), sizeof(int16_t));
            ...
            pmemobj_persist(bt->pop, (char*)&(hdr.leftmost_ptr), sizeof(page *));
          ...
          hdr.is_deleted = 1;
          pmemobj_persist(bt->pop, (char*)&(hdr.is_deleted), sizeof(uint8_t));
          ...
          if(hdr.leftmost_ptr == nullptr){
            for(int i=0; i<num_dist_entries; i++){
              D_RW(left_sibling)->insert_key(bt->pop, records[i].key, records[i].ptr,
                  &left_num_entries); 
            } 
            ...
            pmemobj_persist(bt->pop, (char*)D_RW(new_sibling), sizeof(page));

            D_RW(left_sibling)->hdr.sibling_ptr = new_sibling;
            pmemobj_persist(bt->pop, (char*)&(D_RW(left_sibling)->hdr.sibling_ptr), sizeof(page *));
...
//        struct timespec start, end;
        ...
        if(*num_entries == 0) {  // this page is empty
          ...
          if(flush) {
#ifdef BREAKDOWN
	    clock_gettime(CLOCK_MONOTONIC, &clwb_start);
#endif
            pmemobj_persist(pop, (void *)this, CACHE_LINE_SIZE);
#ifdef BREAKDOWN
	    clock_gettime(CLOCK_MONOTONIC, &clwb_end);
	    clwbTime += (clwb_end.tv_nsec - clwb_start.tv_nsec) + 1000000000*(clwb_end.tv_sec - clwb_start.tv_sec);
#endif
			//clflush_cnt++;
          }
        }
...
//          clock_gettime(CLOCK_MONOTONIC,&start);
	    ...
	    clock_gettime(CLOCK_MONOTONIC, &clwb_start);
#endif
              pmemobj_persist(pop, (void *)&records[*num_entries+1].ptr, sizeof(char*));
#ifdef BREAKDOWN
	    clock_gettime(CLOCK_MONOTONIC, &clwb_end);
	    clwbTime += (clwb_end.tv_nsec - clwb_start.tv_nsec) + 1000000000*(clwb_end.tv_sec - clwb_start.tv_sec);
#endif
	    ...
	    clock_gettime(CLOCK_MONOTONIC, &clwb_start);
#endif
                pmemobj_persist(pop, (void *)&records[i+1], sizeof(entry));
#ifdef BREAKDOWN
	    clock_gettime(CLOCK_MONOTONIC, &clwb_end);
	    clwbTime += (clwb_end.tv_nsec - clwb_start.tv_nsec) + 1000000000*(clwb_end.tv_sec - clwb_start.tv_sec);
#endif
	    ...
	    clock_gettime(CLOCK_MONOTONIC, &clwb_start);
#endif
              pmemobj_persist(pop, (void *)&records[0], sizeof(entry));
#ifdef BREAKDOWN
	    clock_gettime(CLOCK_MONOTONIC, &clwb_end);
	    clwbTime += (clwb_end.tv_nsec - clwb_start.tv_nsec) + 1000000000*(clwb_end.tv_sec - clwb_start.tv_sec);
#endif
...
//		  split_cnt++;
          ...
          if(hdr.leftmost_ptr == NULL){ // leaf node
            for(i=m, j=0; i<num_entries; ++i, ++j){ 
              sibling_ptr->records[j] = records[i];
            }
          }
          else{ // internal node
            for(i=m+1, j=0;i<num_entries;++i, ++j){ 
              sibling_ptr->records[j] = records[i];
            }
          ...
          sibling_cnt = j;
          sibling_ptr->hdr.last_index = j-1;
	  ...
	  clock_gettime(CLOCK_MONOTONIC, &clwb_start);
#endif
          pmemobj_persist(bt->pop, (void *)&hdr, sizeof(hdr));
#ifdef BREAKDOWN
	  clock_gettime(CLOCK_MONOTONIC, &clwb_end);
	  clwbTime += (clwb_end.tv_nsec - clwb_start.tv_nsec) + 1000000000*(clwb_end.tv_sec - clwb_start.tv_sec);
#endif
	  ...
	  clock_gettime(CLOCK_MONOTONIC, &clwb_start);
#endif
          pmemobj_persist(bt->pop, (void *)&records[m], sizeof(entry));
#ifdef BREAKDOWN
	  clock_gettime(CLOCK_MONOTONIC, &clwb_end);
	  clwbTime += (clwb_end.tv_nsec - clwb_start.tv_nsec) + 1000000000*(clwb_end.tv_sec - clwb_start.tv_sec);
#endif
	  ...
	  clock_gettime(CLOCK_MONOTONIC, &clwb_start);
#endif
          pmemobj_persist(bt->pop, (void *)&hdr.last_index, sizeof(int16_t));
#ifdef BREAKDOWN
	  clock_gettime(CLOCK_MONOTONIC, &clwb_end);
	  clwbTime += (clwb_end.tv_nsec - clwb_start.tv_nsec) + 1000000000*(clwb_end.tv_sec - clwb_start.tv_sec);
#endif
...
//            keyinsert_time += (end.tv_sec - start.tv_sec) * 1000000000 + (end.tv_nsec - start.tv_nsec);
    ...
    void print() {
      ...
      printf("%lld ", hdr.sibling_ptr.oid.off);
...
void btree::setNewRoot(TOID(page) new_root) {
  this->root = new_root;
  pmemobj_persist(pop, (void *)&root, sizeof(TOID(page)));
  ++height;
}
...
//  clock_gettime(CLOCK_MONOTONIC,&start);
  ...
  insertTime += (eend.tv_nsec - sstart.tv_nsec) + 1000000000*(eend.tv_sec - sstart.tv_sec);
#endif
//  clock_gettime(CLOCK_MONOTONIC,&end);
//  insert_time += (end.tv_sec - start.tv_sec) * 1000000000 + (end.tv_nsec - start.tv_nsec);
}
