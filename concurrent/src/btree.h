...
//            page* new_root = new page(left_sibling, parent_key, new_sibling, hdr.level + 1);
          ...
          D_RW(new_sibling)->hdr.mtx->unlock();
        }
      }
      else {
        hdr.is_deleted = 1;
	...
	hdr.mtx->lock_shared();
        ...
        if(ret) {
	  hdr.mtx->unlock_shared();
          return ret;
        }

        if((t = (char *)hdr.sibling_ptr.oid.off) && key >=
    		D_RW(hdr.sibling_ptr)->records[0].key) {
	  hdr.mtx->unlock_shared();
          return t;
    	}
...
void btree::setNewRoot(TOID(page) new_root) {
  root = new_root;
   pmemobj_persist(pop, &root,sizeof(char*));
  ++height;
}
